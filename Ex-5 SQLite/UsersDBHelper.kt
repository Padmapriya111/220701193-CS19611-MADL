...
// Create a new map of values, where column names are the keys
        ...
        values.put(
            DBContract.UserEntry.COLUMN_REGISTER_NUMBER,
            user.registernumber)
...
// Insert the new row, returning the primary key value of the new row
    ...
    fun readUser(registerNumber : String): ArrayList<UserModel> {
        ...
        try {
            cursor = db.rawQuery( DBContract.UserEntry.TABLE_NAME +
                    "select * from "
                    +
                    " WHERE "
                    +
                    DBContract.UserEntry.COLUMN_REGISTER_NUMBER + " = '" + registerNumber + "'", null)
        }
        ...
        if (cursor!!.moveToFirst()) {
            while (cursor.isAfterLast == false) {
                ...
                cgpa = cursor.getString(cursor.getColumnIndex(DBContract.UserEntry.COLUMN_CGPA))
