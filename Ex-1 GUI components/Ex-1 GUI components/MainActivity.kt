...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btFontColor.setOnClickListener {
            when (fontColor % 3) {
                0 -> tvText.setTextColor(Color.GREEN)
                1 -> tvText.setTextColor(Color.RED)
        ...
        btBackgroundColor.setOnClickListener {
            when (bgColor % 3) {
                0 -> linearlayout.setBackgroundColor(Color.BLUE)
                1 -> linearlayout.setBackgroundColor(Color.GREEN)
                2 -> linearlayout.setBackgroundColor(Color.RED)
            }
            bgColor++
        }
    }
